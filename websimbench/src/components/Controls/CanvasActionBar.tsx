import { Trash, Cube, Play, Stop, Users, Speedometer } from "@phosphor-icons/react";
...
export const CanvasActionBar = ({
    ...
    return (
        <div className="canvas-action-bar" onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
            {/* Agent Control */}
            {!isAgentCountDefined && (
                    ...
                    <div className="action-bar-group">
                        <Button
                            ...
                            className={cn(
                                ...
                                isPlacing ? "obstacle-btn-active-place" : "obstacle-btn-inactive"
                            )}
                        >
            ...
            <div className="action-bar-group">
                <Button
                    ...
                    title={!isRunning && !canRun ? "Add simulation code to run." : undefined}
                    size="sm"
                    className={cn(
                        "action-bar-run-btn",
                ...
                {fps !== undefined && (
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-black/40 rounded-md border border-white/10 ml-2">
                        <Speedometer size={14} weight="fill" className="text-tropicalTeal" />
                        <span className="font-mono text-sm font-bold text-white tracking-tight">{fps}</span>
