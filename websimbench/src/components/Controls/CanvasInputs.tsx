...
export const CanvasInputs = ({ inputs, definedInputs, handleInputChange }: CanvasInputsProps) => {
    ...
    return (
        <div className="canvas-inputs-container" onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
            {availableInputs.map((def) => (
                <div key={def.name} className="canvas-inputs-item">
                    <span className="canvas-inputs-label">{formatInputName(def.name)}</span>
                    <ScrubbableInput
