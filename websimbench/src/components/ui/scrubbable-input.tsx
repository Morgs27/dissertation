...
interface ScrubbableInputProps extends Omit<React.ComponentProps<"input">, "onChange"> {
    value: number
...
const ScrubbableInput = React.forwardRef<HTMLInputElement, ScrubbableInputProps>(
    ({ className, value, onChange, min = -Infinity, max = Infinity, step, sensitivity = 1, ...props }, ref) => {
        ...
        const effectiveStep = React.useMemo(() => {
            ...
            if (Number.isInteger(value) && Number.isInteger(min) && Number.isInteger(max)) return 1;
        ...
        const handlePointerMove = (e: React.PointerEvent) => {
            ...
            if ((e.currentTarget as HTMLElement).hasPointerCapture(e.pointerId)) {
                ...
                if (isDragging) {
                    ...
                    let newValue = startValueRef.current + (steps * effectiveStep * multiplier * sensitivity);
        ...
        return (
            <div className="relative group w-full">
                ...
                />
                {!isEditing && (
                    <ArrowsLeftRight weight="bold" size={12} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/20 pointer-events-none group-hover:text-tropicalTeal transition-colors" />
                )}
            </div>
        )
    }
