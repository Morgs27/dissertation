          ...
          <div className="flex flex-col gap-1">
            {filteredLogs.map((log, i) => (
              <div key={i} className="log-item">
                <span className="log-timestamp">[{new Date(log.timestamp).toLocaleTimeString([], { hour12: false })}]</span>
                <span className={`log-context ${log.level === 'Error' ? 'text-red-400' :
                  log.level === 'Warning' ? 'text-orange-400' :
                    'text-tropicalTeal'
                  }`}>[{log.context}]</span>
                <span className={`log-message ${log.level === 'Error' ? 'text-red-300' :
