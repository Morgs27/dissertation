...
import { AVAILABLE_COMMANDS_LIST } from '../../src/simulation/compiler/parser';
    ...
    describe('all backends produce output', () => {
        for (const [name, source] of Object.entries(SIMULATIONS)) {
            it(`${name}: all 3 backends produce non-empty output`, () => {
                ...
                expect(result.jsCode.length, `JS empty for ${name}`).toBeGreaterThan(50);
                expect(result.wgslCode.length, `WGSL empty for ${name}`).toBeGreaterThan(50);
                expect(result.WASMCode.length, `WAT empty for ${name}`).toBeGreaterThan(50);
            });
        }
    });
    ...
    describe('micro-simulations: all 3 backends produce output', () => {
        for (const [name, source] of Object.entries(MICRO_SIMULATIONS)) {
            it(`${name}: all 3 backends produce non-empty output`, () => {
                ...
                expect(result.jsCode.length, `JS empty for ${name}`).toBeGreaterThan(10);
                expect(result.wgslCode.length, `WGSL empty for ${name}`).toBeGreaterThan(10);
                expect(result.WASMCode.length, `WAT empty for ${name}`).toBeGreaterThan(10);
            });
        }
    });
    ...
    describe('input extraction consistency', () => {
        for (const [name, source] of Object.entries(SIMULATIONS)) {
            it(`${name}: requiredInputs is consistent across compilations`, () => {
                ...
                expect(result1.requiredInputs.sort()).toEqual(result2.requiredInputs.sort());
                expect(result1.definedInputs.map(i => i.name).sort())
                    .toEqual(result2.definedInputs.map(i => i.name).sort());
            });
        }
    });
    ...
    describe('command coverage across backends', () => {
        ...
        for (const [cmd, source] of Object.entries(commandTestCases)) {
            it(`${cmd}: compiles in all 3 backends without error`, () => {
                const compiler = new Compiler();
                expect(() => compiler.compileAgentCode(source),
                    `Compilation failed for command: ${cmd}`).not.toThrow();
            });
        }
    });
