(agent, inputs) => {
        ...
        let here = _sense(f(0), f(0)); 
        if ((here < f(inputs.threshold))) {
    
        const __c = f(Math.cos(f(-f(1.5708)))); const __s = f(Math.sin(f(-f(1.5708)))); const __vx = f(f(vx * __c) - f(vy * __s)); vy = f(f(vx * __s) + f(vy * __c)); vx = __vx;
        _deposit(f(inputs.depositAmount));
        }
        else {
        const __c = f(Math.cos(f(1.5708))); const __s = f(Math.sin(f(1.5708))); const __vx = f(f(vx * __c) - f(vy * __s)); vy = f(f(vx * __s) + f(vy * __c)); vx = __vx;
        }
