  ...
  (global $inputs_wanderAmount (export "inputs_wanderAmount") (mut f32) (f32.const 0))
  ...
  (global $inputs_height (export "inputs_height") (mut f32) (f32.const 0))
    ...
    (if (i32.and (f32.gt (local.get $sF) (local.get $sL)) (f32.gt (local.get $sF) (local.get $sR))) (then
    )
    (else (if (f32.gt (local.get $sL) (local.get $sR)) (then
    (local.set $__c (call $cos (global.get $inputs_turnAngle)))
            (local.set $__s (call $sin (global.get $inputs_turnAngle)))
    ...
    (else (if (f32.gt (local.get $sR) (local.get $sL)) (then
    (local.set $__c (call $cos (f32.neg (global.get $inputs_turnAngle))))
            (local.set $__s (call $sin (f32.neg (global.get $inputs_turnAngle))))
            (local.set $__vx (f32.sub (f32.mul (local.get $vx) (local.get $__c)) (f32.mul (local.get $vy) (local.get $__s))))
            (local.set $vy (f32.add (f32.mul (local.get $vx) (local.get $__s)) (f32.mul (local.get $vy) (local.get $__c))))
            (local.set $vx (local.get $__vx))
    )
    (else
    (local.set $wander (f32.mul (f32.sub (local.get $r) (f32.const 0.5)) (global.get $inputs_wanderAmount)))
    (local.set $__c (call $cos (local.get $wander)))
            (local.set $__s (call $sin (local.get $wander)))
            (local.set $__vx (f32.sub (f32.mul (local.get $vx) (local.get $__c)) (f32.mul (local.get $vy) (local.get $__s))))
            (local.set $vy (f32.add (f32.mul (local.get $vx) (local.get $__s)) (f32.mul (local.get $vy) (local.get $__c))))
            (local.set $vx (local.get $__vx))
    ))))))
