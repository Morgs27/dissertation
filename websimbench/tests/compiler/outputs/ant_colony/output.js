(agent, inputs) => {
        ...
        let wander = f(f(r - f(0.5)) * f(inputs.wanderAmount)); 
        const __c = f(Math.cos(wander)); const __s = f(Math.sin(wander)); const __vx = f(f(vx * __c) - f(vy * __s)); vy = f(f(vx * __s) + f(vy * __c)); vx = __vx;
        }
