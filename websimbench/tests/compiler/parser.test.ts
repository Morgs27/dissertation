...
import { DSLParser, AVAILABLE_COMMANDS_LIST } from '../../src/simulation/compiler/parser';
import type { ParsedLineType } from '../../src/simulation/compiler/parser';
...
describe('DSLParser', () => {
    ...
    describe('parseDSLLine', () => {
        ...
        describe('var declarations', () => {
            ...
            it('should parse var with complex expression', () => {
                ...
                expect(result).toEqual({ type: 'var', name: 'dist2', expression: 'dx*dx + dy*dy' });
            });

            it('should parse var with function call', () => {
                const result = DSLParser.parseDSLLine('var nearby = neighbors(inputs.perceptionRadius);');
            ...
            it('should parse var with mean() call', () => {
                const result = DSLParser.parseDSLLine('var avgVx = mean(nearbyAgents.vx);');
                expect(result).toEqual({
                    type: 'var',
                    name: 'avgVx',
            ...
            it('should parse var with sense() call', () => {
                const result = DSLParser.parseDSLLine('var sL = sense(inputs.sensorAngle, inputs.sensorDist);');
                expect(result).toEqual({
                    type: 'var',
                    name: 'sL',
            ...
            it('should parse var with random() call', () => {
                ...
                expect(result).toEqual({ type: 'var', name: 'r', expression: 'random()' });
            });
            ...
            it('should parse var with negative value', () => {
                ...
                expect(result).toEqual({ type: 'var', name: 'dx', expression: 'x - neighbor_x' });
            });
        });
        ...
        describe('if statements', () => {
            ...
            it('should parse if with complex condition', () => {
                const result = DSLParser.parseDSLLine('if (dist2 < inputs.separationDist^2 && dist2 > 0) {');
                expect(result).toEqual({
                    type: 'if',
        ...
        describe('for loops', () => {
            it('should parse standard for loop', () => {
                const result = DSLParser.parseDSLLine('for (var i = 0; i < nearbyAgents.length; i++) {');
                expect(result).toEqual({
                    type: 'for',
                    init: 'var i = 0',
        ...
        describe('foreach loops', () => {
            it('should parse foreach with as keyword', () => {
                const result = DSLParser.parseDSLLine('foreach (nearbyAgents as neighbor) {');
                expect(result).toEqual({
                    type: 'foreach',
        ...
        describe('commands', () => {
            ...
            it('should parse moveForward', () => {
                ...
                expect(result).toEqual({ type: 'command', command: 'moveForward', argument: 'inputs.speed' });
            });

            it('should parse deposit', () => {
                const result = DSLParser.parseDSLLine('deposit(1.0);');
                expect(result).toEqual({ type: 'command', command: 'deposit', argument: '1.0' });
            });

            it('should parse sense with two args', () => {
                const result = DSLParser.parseDSLLine('sense(inputs.sensorAngle, inputs.sensorDist);');
                expect(result).toEqual({
                    type: 'command',
                    command: 'sense',
            ...
            it('should parse enableTrails with two args', () => {
                const result = DSLParser.parseDSLLine('enableTrails(inputs.depositAmount, inputs.decayFactor);');
                expect(result).toEqual({
                    type: 'command',
            ...
            it('should parse species', () => {
                ...
                expect(result).toEqual({ type: 'command', command: 'species', argument: '3' });
            });

            it('should parse print', () => {
                const result = DSLParser.parseDSLLine('print(x);');
                expect(result).toEqual({ type: 'command', command: 'print', argument: 'x' });
            });

            it('should parse avoidObstacles', () => {
                const result = DSLParser.parseDSLLine('avoidObstacles(1.0);');
                expect(result).toEqual({ type: 'command', command: 'avoidObstacles', argument: '1.0' });
            });
        });
    ...
    describe('parseCommandLine', () => {
        ...
        it('should handle multiple comma-separated arguments', () => {
            const result = DSLParser.parseCommandLine('sense(inputs.angle, inputs.dist)');
            expect(result).not.toBeNull();
    ...
    describe('applyCommandTemplate', () => {
        it('should replace single {arg}', () => {
            expect(DSLParser.applyCommandTemplate('y = y - {arg};', '5')).toBe('y = y - 5;');
        });

        it('should replace multiple {arg}', () => {
            expect(DSLParser.applyCommandTemplate('{arg} * {arg}', '3')).toBe('3 * 3');
        });

        it('should handle complex arg expression', () => {
            expect(DSLParser.applyCommandTemplate('x = x + {arg};', 'inputs.speed * dt'))
    ...
    describe('parseLines', () => {
        it('should parse multiple command lines', () => {
            const commandMap: Record<string, string> = {
                ...
                moveLeft: 'x = x - {arg};',
                moveRight: 'x = x + {arg};',
                addVelocityX: '', addVelocityY: '', setVelocityX: '', setVelocityY: '',
                updatePosition: '', borderWrapping: '', borderBounce: '', limitSpeed: '',
                turn: '', moveForward: '', sense: '', deposit: '', enableTrails: '',
                print: '', species: '', avoidObstacles: '',
            };
