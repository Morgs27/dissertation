 ...
 */
function parseBoidsDSL(lines: string[]): string[] {
    ...
    for (const line of lines) {
        ...
        const transpiled = transpileLine(line);
        if (transpiled !== null) {
...
export const compileDSLtoJS = (lines: string[], _inputs: string[], logger: Logger): string => {
    // Try new parser for boids-style DSL
    const boidStatements = parseBoidsDSL(lines);
