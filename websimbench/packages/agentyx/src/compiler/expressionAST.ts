...
class Parser {
    ...
    private parseExpression(maxPrec: number): ExprNode {
        ...
        while (this.peek() && PRECEDENCE[this.peek()!] !== undefined && PRECEDENCE[this.peek()!] <= maxPrec) {
 ...
 */
export function generateJS(node: ExprNode, wrapArithmetic: boolean = true, randomInputs: Set<string> = new Set()): string {
    switch (node.type) {
        case 'number':
        ...
        case 'property': {
            ...
            if ((node.object as { type: 'identifier'; name: string }).name === 'inputs') {
            ...
            switch (node.name) {
                ...
                case 'mean':
                    ...
                    if (node.args.length === 1 && node.args[0].type === 'property') {
                        const prop = node.args[0] as { type: 'property'; object: ExprNode; property: string };
 ...
 */
export function transformExpression(expr: string, randomInputs: Set<string> = new Set()): string {
