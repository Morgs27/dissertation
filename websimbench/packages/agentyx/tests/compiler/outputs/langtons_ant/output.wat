  ...
  (global $inputs_threshold (export "inputs_threshold") (mut f32) (f32.const 0))
  ...
  (global $inputs_height (export "inputs_height") (mut f32) (f32.const 0))
    ...
    ;; execute DSL
    nop
    (local.set $here (call $sense (local.get $x) (local.get $y) (local.get $vx) (local.get $vy) (f32.const 0) (f32.const 0)))
    (if (f32.lt (local.get $here) (global.get $inputs_threshold)) (then
    (local.set $__c (call $cos (f32.const -1.5708)))
            (local.set $__s (call $sin (f32.const -1.5708)))
