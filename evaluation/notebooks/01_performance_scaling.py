...
STYLE_MAP = {
    "JavaScript": {
        "color": get_method_color("JavaScript"),
        "linewidth": 1.8,
        "linestyle": "-",
        "marker": "o",
        "markersize": 3,
    ...
    "Web Workers": {
        "color": get_method_color("WebWorkers"),
        "linewidth": 1.8,
        "linestyle": "-",
        "marker": "o",
        "markersize": 3,
    ...
    "WebAssembly": {
        "color": get_method_color("WebAssembly"),
        "linewidth": 1.8,
        "linestyle": "-.",
        "marker": "o",
        "markersize": 3,
...
for ax, sim in zip(axes, sweep_sims):
    ...
    # Much lighter grid
    ax.grid(True, which="major", linestyle=":", linewidth=0.5, alpha=0.18)
    ax.grid(True, which="minor", linestyle=":", linewidth=0.35, alpha=0.05)
...
fig.suptitle(
    "Mean Compute Time vs Agent Count Across Simulation Workloads",
    fontsize=15,
    fontweight="bold",
    y=1.08
)

fig.supxlabel("Agent Count", fontsize=12)
fig.supylabel("Mean Compute Time (ms)", fontsize=12)
...
if overlapping_sims:
    ...
    for i, sim in enumerate(overlapping_sims):
        ...
        for method in METHOD_ORDER:
            subset = sim_continuous[
                sim_continuous["method"] == method
        ...
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_title(sim.capitalize(), fontsize=12, fontweight="bold")
        ax.xaxis.set_major_formatter(ticker.FuncFormatter(format_km))
        ...
        # Subtle transition line between main and high-range datasets
        max_base = sim_base["agentCount"].max()
        if pd.notna(max_base):
            ax.axvline(
                max_base,
    ...
    fig.suptitle(
        "Continuous Compute Scaling Across Simulation Workloads",
        fontsize=15,
        fontweight="bold",
        y=1.08,
    )
...
for ax, sim in zip(axes, sweep_sims):
    ...
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_title(sim.capitalize(), fontsize=16, fontweight="bold", pad=10)
...
fig.supxlabel("Agent Count", fontsize=20, y=0.04)
fig.supylabel("Mean Compute Time (ms)", fontsize=20, x=0.04)
...
if overlapping_sims:
    ...
    for i, sim in enumerate(overlapping_sims):
        ...
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_title(sim.capitalize(), fontsize=20, fontweight="bold", pad=10)
        ax.xaxis.set_major_formatter(ticker.FuncFormatter(format_km))
    ...
    fig.supxlabel("Agent Count", fontsize=20, y=0.05)
    fig.supylabel("Mean Compute Time (ms)", fontsize=20, x=0.04)
